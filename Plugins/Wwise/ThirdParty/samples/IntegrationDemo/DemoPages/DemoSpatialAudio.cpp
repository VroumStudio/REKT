...
bool DemoSpatialAudio::Init_PortalsDemo()
{
	...
	m_szHelp =
		...
		"Finally, this demo shows how to use a Room to play multi-channel ambient sounds / room tones that contract and become point-source at portals.\n"
...
bool DemoSpatialAudio::Update()
{
	...
	if ( m_bMoved )
	{
		...
		AK::SpatialAudio::SetEmitterObstructionAndOcclusion(EMITTER, obsEmitter, 0.0f);
		...
		float dryDiffraction = FLT_MAX;
		float wetDiffraction = FLT_MAX;
		...
		if (m_numLines > 0)
		{
			...
			for (AkUInt32 path = 0; path < uNumPaths; path++)
			{
				...
				// Listener to node 0.
				// Do not draw this line if this portal is obstructed (per our raycast obstruction algorithm above).
				if (m_obsPortal[PortalIDToIndex(paths[path].portals[0])] == 0.f)
				{
					m_aLines[line] = new Line(*this);
					m_aLines[line]->SetPoints(AKPosToPixels_X(listenerPos.X), AKPosToPixels_Y(listenerPos.Z), AKPosToPixels_X(paths[path].nodePoint[0].X), AKPosToPixels_Y(paths[path].nodePoint[0].Z));
				}
				else
				{
					m_aLines[line] = NULL;
				...
				{
					m_aLines[line] = new Line(*this);
					m_aLines[line]->SetPoints(AKPosToPixels_X(paths[path].nodePoint[i - 1].X), AKPosToPixels_Y(paths[path].nodePoint[i - 1].Z), AKPosToPixels_X(paths[path].nodePoint[i].X), AKPosToPixels_Y(paths[path].nodePoint[i].Z));
					++line;
				}
				*/
				...
				m_aLines[line] = new Line(*this);
				m_aLines[line]->SetPoints(AKPosToPixels_X(paths[path].nodePoint[numNodes - 1].X), AKPosToPixels_Y(paths[path].nodePoint[numNodes - 1].Z), AKPosToPixels_X(emitterPos.X), AKPosToPixels_Y(emitterPos.Z));
				++line;

				// Display the minimum diffraction amount of the (possibly) two propagation paths. Since we only have Room (i.e. game object) scoped RTPCs in Wwise at the moment, this is the actual value that will be pushed
				// to our built-in game parameter "Diffraction".
				dryDiffraction = AkMin(dryDiffraction, paths[path].dryDiffraction);
				wetDiffraction = AkMin(wetDiffraction, paths[path].wetDiffraction);
			}
			m_dryDiffraction = dryDiffraction;
			m_wetDiffraction = wetDiffraction;
		}
		else
		{
			m_dryDiffraction = 0;
...
void DemoSpatialAudio::Draw()
{
	...
	char strBuf[128];
	if (m_numLines)
		snprintf(strBuf, 128, "X: %.2f\nZ: %.2f\nDiffraction dry: %.2f\nDiffraction wet: %.2f", m_fGameObjectX, m_fGameObjectZ, m_dryDiffraction, m_wetDiffraction);
	else
...
void DemoSpatialAudio::SetPortalsObstruction()
{
	...
	AK::SpatialAudio::SetPortalObstructionAndOcclusion(PORTAL0, m_obsPortal[0], 0.f);
	AK::SpatialAudio::SetPortalObstructionAndOcclusion(PORTAL1, m_obsPortal[1], 0.f);
}
